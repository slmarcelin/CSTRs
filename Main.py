...
def SecondLoop():
	...
	if ard.run :
		...
		for i in rng:
			...
			op.append(Reactor[i].OP)
			rpm.append(Reactor[i].RPM)

		#ard.ControlHeaters(ard=A, react = i, op=op) #CONTROL HEATER
		ard.ControlStirringMotors(A,rpm=rpm, en=True) #CONTROL MOTORS
...
def _PIDWindow():
	...
	# Enter button
	PID_EnterButton= tk.Button(top, text= "Enter" , command=enter_btn) 
	PID_EnterButton.grid(row=5, column=2)
...
GUI_window.title('CSRT Reactors control')
#GUI_window.iconbitmap()
...
GUI_FeedUnit= tk.Entry(GUI_FeedFrame, font= font_small)
