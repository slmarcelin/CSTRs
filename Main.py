...
def ToggleStirr():
    ...
    for i in range(0,6):
        if StirrStat[i]: #Stirring is on
            if (ActivTime[i]+ TOn[i] <= time.time()): # if it is time to turn off then                
                StirrStat[i]= not StirrStat[i]# turn off

                ActivTime[i]=time.time()# Save current time
        else: # stirr is off
            if (ActivTime[i]+ TOff[i] <= time.time()):# if it is time to turn on then
                StirrStat[i]= not StirrStat[i]# turn on
                ActivTime[i]=time.time()# save current time
def UpdateLabels():
    global LoopTimer
    ...
    print (PV)
    for i in range(0,6):
        TextVarPV[i].set(PV[i])
        TextVarSP[i].set(SP[i])
        TextVarOP[i].set(OP[i])
        TextVarOP[i].set('Heating {:0.00f} %'.format(OP[i]))

        TextVarOnT[i].set(TOn[i])
        TextVarOff[i].set(TOff[i])
        TextVarStat[i].set(BoolToText(StirrStat[i],'ON','OFF'))

        TextVarTime[i].set('{:00.0f}'.format(time.time()-ActivTime[i]))
        TextVarTime[i].set(strftime("%H:%M:%S", gmtime(time.time()-ActivTime[i])))
...
for row in range(0, 3):  # 3 rows
    for col in range(0, 2):  # 2columns
        ...
        #PEnButton[i].place(relx=0.8, rely=0.0, relwidth=0.2, relheight=0.15)
